...
- **ShuttleIQ Marketplace**: Player-facing booking and community platform with:
    ...
    - **Stripe Payment Integration**: Real payment processing via Stripe PaymentIntents with in-app Stripe Elements checkout. Booking flow: user clicks "Book & Pay Now" → navigates to `/marketplace/checkout/:sessionId` → selects payment method (card or cash) → for card: backend creates PaymentIntent + pending booking → user enters card details via Stripe Elements → payment confirmed → booking status updated to "confirmed". For cash: booking confirmed immediately with `paymentMethod: 'cash'` and `cashPaid: false` → admin can toggle `cashPaid` status in the bookings sheet. Cancellation policy warning shown on checkout page (cancellations within 12h subject to full payment, last-hour cancellations subject to full payment, no-shows charged 150% of session price). Key files: `server/stripeClient.ts`, `server/webhookHandlers.ts`, `server/marketplace-routes.ts`, `client/src/pages/marketplace/Checkout.tsx`. Webhook routes (`/api/stripe/webhook` and `/api/marketplace/stripe/webhook`) registered BEFORE `express.json()` in `server/index.ts`. Uses `stripe-replit-sync` for Stripe schema sync and managed webhooks. Webhook handles both `checkout.session.completed` and `payment_intent.succeeded` events. Checkout success/cancel fallback pages at `/marketplace/checkout/success` and `/marketplace/checkout/cancel`.
