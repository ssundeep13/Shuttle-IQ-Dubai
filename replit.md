...
- **SEO indexing policy**: Canonical noindex route list lives in `shared/seo.ts` (`NOINDEX_PATH_PATTERNS`, `ROBOTS_TXT_DISALLOW`, `isNoindexPath`). Three sources stay in sync via parity tests in `tests/seo-robots-meta.test.tsx`: (1) Express middleware in `server/index.ts` emits `X-Robots-Tag: noindex, nofollow` for non-API requests matching the list (covers non-JS crawlers); (2) `client/src/components/RobotsMetaController.tsx` mounted in `App.tsx` watches `useLocation` and uses `usePageMeta` to override the default `<meta name="robots" content="index,follow">` (set in `client/index.html`) to `noindex,nofollow` on listed routes (covers JS-rendering crawlers); (3) `client/public/robots.txt` Disallow lines. Brand wordmark `<Link>`s in `MarketplaceNav` and `MarketplaceFooter` carry `aria-label="ShuttleIQ home"` so Lighthouse's "Links do not have descriptive text" audit passes.
- **Cancel Event (Admin)**: One-click "Cancel event" on the admin Sessions tab (`SessionCard` Cancel button + AlertDialog) calls `POST /api/marketplace/admin/sessions/:id/cancel`. Storage helper `cancelBookableSessionAndRefund(id)` runs in a single `db.transaction`: cancels every non-cancelled booking with `cancellationReason='event_cancelled_by_admin'`, inserts a `refund_required` notification per Ziina/cash-paid booking, and marks the bookable session `status='cancelled'` last so any failure rolls back. Idempotent — repeat calls return `{alreadyCancelled:true}`. Route then loops `affectedBookings` to credit wallets (`refundBookingWalletCredit`) and email each booker via `sendCancellationEmail`. Cancelled bookable sessions are filtered out of `getUpcomingBookableSessions` and shown with a destructive "Cancelled" badge in admin. Bookings now have a nullable `cancellationReason` column. UI auto-switches to the Refunds tab on success.
