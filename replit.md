...
- **Marketplace**: Player-facing platform for session browsing and booking, including Google OAuth, Ziina payment integration, player dashboards (My Bookings, My Scores, Rankings, Profile), community tagging (Player Personalities), and community tag suggestions with voting. Marketplace users can self-service update their account email/phone from /marketplace/profile via OTP-to-new-value verification (audited in `marketplace_user_contact_changes`).
