...
- **ShuttleIQ Marketplace**: Player-facing booking and community platform with:
    ...
    - **Tap Payment Integration**: Real payment processing via Tap Payments (UAE/GCC-focused) with in-app card widget. Booking flow: user clicks "Book & Pay Now" → navigates to `/marketplace/checkout/:sessionId` → selects payment method (card or cash) → for card: frontend tokenizes card via Tap.js → backend creates charge + pending booking → payment status checked and booking confirmed. For cash: booking confirmed immediately with `paymentMethod: 'cash'` and `cashPaid: false` → admin can toggle `cashPaid` status in the bookings sheet. Cancellation policy warning shown on checkout page (cancellations within 12h subject to full payment, last-hour cancellations subject to full payment, no-shows charged 150% of session price). Key files: `server/tapClient.ts`, `server/tapWebhookHandlers.ts`, `server/marketplace-routes.ts`, `client/src/pages/marketplace/Checkout.tsx`. Webhook route (`/api/marketplace/tap/webhook`) registered BEFORE `express.json()` in `server/index.ts`. Requires `TAP_SECRET_KEY` and `TAP_PUBLIC_KEY` environment variables. Checkout success/cancel fallback pages at `/marketplace/checkout/success` and `/marketplace/checkout/cancel`.
...
#### Payments
- **Tap Payments**: Payment processing via Tap Payments REST API (UAE/GCC-focused). Uses `TAP_SECRET_KEY` and `TAP_PUBLIC_KEY` env vars. Frontend loads `tap.min.js` SDK dynamically from `https://secure.gosell.io/js/sdk/tap.min.js`.
