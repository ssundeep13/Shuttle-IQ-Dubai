...
// ─── Refund Processed (#231) ──────────────────────────────────────────────
// Sent only on TERMINAL refund success (synchronous Ziina success OR the
// refund.completed webhook). amountAed is the cash amount refunded via Ziina
// (never the wallet-paid portion, which is returned to the wallet separately).
// Swallows errors — a failed email must never block or reverse a refund.
